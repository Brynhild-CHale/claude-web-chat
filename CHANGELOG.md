...
- **A service can no longer approve itself.** The approve/deny decision travelled as an ordinary store key, which any pane can write. It is now a dedicated `service:decision` WebSocket frame, validated against a server-issued nonce, that pane JavaScript has no way to send.
