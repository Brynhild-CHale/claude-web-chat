...
const { safeHref, listItems } = require('./profiles/util');
...
// Resolve a possibly-relative URL against the captured page URL and gate its
// scheme. One gate for the whole slice: util's safeHref, which normalises
// through `new URL` before reading the protocol. The private regex test that
// used to live here was bypassable with no base — `java\tscript:alert(1)` reached
// an href verbatim, and `url` is optional in the /api/capture body — while its
// resolveUrl half passed an unresolvable value through untouched.
//
// A bare fragment (no baseUrl) keeps working: an absolute http(s)/mailto/tel URL
// needs no base, and a relative one has nowhere to point, so it drops.
function safeUrl(u, base) {
  return safeHref(u, base);
}
...
function mdList(el, ordered, depth, opts) {
  ...
  const items = listItems(el); // own <li> children only — nested lists recurse below
  const lines = [];
  items.forEach((li, i) => {
