...
// capture/pane.js renders the reader-lite simplified document (simplify.js) into
...
// links (resolved against the page URL, scheme-gated) — links are preserved as
// { href, text } so the distilled content stays navigable without carrying raw
// markup.
function collectInline(node, base, out) {
  for (const child of node.childNodes) {
    if (child.nodeType === 3) { out.text += child.text; continue; }
...
function walk(node, base, blocks, budget) {
  for (const child of node.childNodes) {
    ...
    if (/^h[1-6]$/.test(tag)) {
      ...
      const items = listItems(child).map((li) => collapse(li.text)).filter(Boolean);
      if (items.length) {
...
module.exports = {
  ...
  // Passive builtins carry a `simplified_pane` flag so capture/pane.js renders
  ...
  extract({ url, root }) {
    ...
    walk(container, url, blocks, budget);
