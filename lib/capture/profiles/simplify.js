...
// in the capture's distillate — capture/pane.js writes it to a sidecar file
...
const { collapse, safeHref, listItems } = require('./util');
// The one escaper (lib/core/html.js). This file used to carry its own — a
// four-character map (`& < > "`, no apostrophe) used in BOTH text and attribute
// positions, which is the shape the escape ratchet exists to catch. Adopting the
// five-character engine is safe here and nowhere near a byte-compare: the reader
// render is the PANE side only (see the header above), never the distillate.
const { escapeHtml: esc } = require('../../core/html');

// Chrome / non-content containers we never descend into. iframe/object/embed and
// script/style are dropped here, and every href/src goes through util's
// safeHref, so the only schemes this render can emit are http/https/mailto/tel.
// (The comment that used to sit here claimed the shadow mount sanitizes as a
// second line of defence. It does not — nothing downstream of this file
// sanitizes, and `javascript:` hrefs reached the pane and the sidecar for as
// long as absolutize waved them through.)
const SKIP = new Set([
...
// <a> hrefs resolve against the page URL and are scheme-gated; in-page anchors
// and refused schemes degrade to plain text.
function inlineHtml(node, base) {
  let out = '';
  for (const child of node.childNodes) {
    if (child.nodeType === 3) { out += esc(child.text); continue; }
    if (child.nodeType !== 1) continue;
    const tag = (child.rawTagName || '').toLowerCase();
    if (tag === 'script' || tag === 'style') continue;
    if (tag === 'br') { out += '<br>'; continue; }
    const inner = inlineHtml(child, base);
    if (tag === 'a') {
      const href = safeHref(child.getAttribute('href'), base);
...
function walk(node, base, parts, budget) {
  for (const child of node.childNodes) {
    ...
    if (/^h[1-6]$/.test(tag)) {
      ...
      const inner = inlineHtml(child, base).trim();
      if (inner) html = `<p>${inner}</p>`;
    } else if (tag === 'ul' || tag === 'ol') {
      const items = listItems(child).map((li) => inlineHtml(li, base).trim()).filter(Boolean);
      ...
      if (img) {
        const src = safeHref(img.getAttribute('src'), base);
...
function simplifyDom(root, { url = '', cap = 200000 } = {}) {
  ...
  walk(container, url, parts, budget);
