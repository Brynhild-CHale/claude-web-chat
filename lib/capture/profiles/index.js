...
const { collapse, absolutize, safeHref, listItems } = require('./util');
...
function loadBundle(dir) {
  ...
  // The reservation is on the RESOLVED name, not just the declared one — a
  // directory named `default` with no "name" in profile.json would otherwise
  // shadow the builtin catch-all for every page. validateMeta already reported
  // the declared spelling; this is the fallback's half of the same rule.
  if (name === 'default' && meta.name !== 'default') {
    errors.push(`profile.json: "default" is reserved for the builtin catch-all (directory name '${fallbackName}')`);
  }
...
//
// `listItems` is on the kit for the same reason: reddit's extractor had
// re-declared it inline (as `ownItems`) because a bundle cannot require, and a
// private copy of a list walker is how the nested-list double-emit bug gets
// reintroduced one bundle at a time. The walker lives in ./util; the kit is how
// a bundle reaches it.
const CTX_HELPERS = { esc: escapeHtml, collapse, absolutize, safeHref, listItems };
