...
module.exports = ({ url, root, esc, safeHref }) => {
  // Resolve against the captured page (an /wiki/… href is document-relative to
  // it); fall back to the canonical host when the capture carried no URL. The
  // injected safeHref is the one gate — this used to be a private absolutize
  // that resolved nothing without a leading slash and passed `javascript:`
  ...
  const absolutize = (href) => safeHref(href, base);
  ...
  const linkify = (node) => {
    if (!node) return '';
    if (node.nodeType === 3) return esc(node.text); // text node (DECODED — rawText leaks `&amp;`/`&nbsp;` into the distillate)
    ...
    if (tag === 'a') {
      ...
      // Self/anchor links and refused schemes degrade to their text.
