...
module.exports = ({ url, root, collapse: clean, safeHref }) => {
  ...
  // Attachment links resolve against the captured page (Gmail's are absolute
  // today; a relative one would otherwise resolve nowhere).
  const pageUrl = (() => {
    try { return new URL(url).href; } catch (_) { return 'https://mail.google.com/'; }
  })();

  // Collect a message's own text, skipping quoted chain / signature / chrome.
  // `.text` is the DECODED text; reading `.rawText` used to leak entities into
  // the body, which this extractor compensated for with a hand-rolled `decode`
  // that mapped six named entities, collapsed every OTHER named entity to a
  // space, and ignored numeric entities entirely. The parser already does the
  // job correctly, so both the leak and the workaround are gone.
  const walkText = (node) => {
    if (!node) return '';
  ...
  for (const card of root.querySelectorAll('.aZo')) {
    ...
    // The download link is the one URL this extractor emits, and it was the one
    // href in the bundled set that never met a gate — the distillate is read by
    // Claude and rendered into the pane, so it goes through the injected
    // safeHref like every other. A refused href leaves the attachment listed
    // with its name, size and type and no link.
    const raw = (a ? a.getAttribute('href') || '' : '').replace(/&amp;/g, '&');
    const url = safeHref(raw, pageUrl) || null;
