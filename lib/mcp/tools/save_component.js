...
module.exports = {
  name: 'save_component',
  description: 'Promote an HTML/JS payload into a named, reusable component stored under `.web-chat/components/<name>/`. After saving, future renders can call `use_component` with this name + params instead of re-emitting the HTML. The `description` is what you read back in `list_components` to decide when to use this — make it specific (purpose, when to use, what params do). Optionally pair a host-side `service` (a service.js driver) so the component reflects LIVE state: while its pane is on the active graph node, the daemon runs the service and it writes the shared store the pane reacts to (git status, test runs, file watches). First run of a service prompts the user to approve it. Reach for a service-backed component whenever the user wants to WATCH or BROWSE live host state rather than get a one-shot answer — and check `list_components` first: builtins (git-dashboard, file-editor) may already cover it. BUILTIN NAMES ARE RESERVED: saving over one (`list_components` marks them `builtin`) is refused with `{ok:false, reserved:true, hint}` — either location, no override, because web-chat could never repair the builtin afterwards. To change what a builtin does, save your version under a DIFFERENT name and mount that.',
  inputSchema: {
    type: 'object',
    properties: {
      name: { type: 'string', description: 'kebab-case component name (`^[a-z][a-z0-9-]*$`) — it becomes a directory name. Builtin names are reserved and refused.' },
