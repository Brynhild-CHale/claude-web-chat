...
async function ensureHub({ maxMs = 6000 } = {}) {
  ...
  if (health && health.role === 'hub') {
    ...
    //
    // If the signal FAILS, the port is not going to free: EPERM means the hub
    // belongs to another uid (the hub port is machine-wide while this registry is
    // per-user — see docs/platform-support.md), ESRCH means it already died and
    // something else holds the port. Swallowing that cost two full maxMs spins
    // and a silent null: the first loop waited for a port that could not free,
    // then the spawned child found the port taken and exited 0, then the second
    // loop waited for a hub that was never coming. Bail immediately and say so.
    if (health.pid) {
      try {
        process.kill(health.pid, 'SIGTERM');
      } catch (e) {
        console.error(`web-chat: the capture hub on port ${port} (pid ${health.pid}) is running an older protocol and cannot be bounced from here: ${e.code || e.message}. `
