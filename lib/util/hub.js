...
const { spawnDetached } = require('./daemon');
...
function spawnHubProcess() {
  // Same detach-with-log as the project daemon, through the same engine — this
  // used to be a second spelling of it, with its own bin-path walk and its own
  // never-closed log fd.
  return spawnDetached({ args: ['hub', 'run'], log: userPaths().hubLog, env: process.env });
}
...
//
// `kill` is injectable for one reason: the EPERM arm below cannot be reached in
// a test without a second uid, so without a seam the difference between "bail"
// and "wait and respawn" is reasoned-correct and unpinned. It is the same single
// signal site, not a second one.
async function ensureHub({ maxMs = 6000, kill = (pid, signal) => process.kill(pid, signal) } = {}) {
  const port = hubPort();
  ...
  if (health && health.role === 'hub') {
    ...
    //
    // Swallowing a failed signal used to cost two full maxMs spins and a silent
    // null: the first loop waited for a port that could not free, the child it
    // spawned found the port taken and exited 0, then the second loop waited for
    // a hub that was never coming.
    //
    // Only EPERM proves the port cannot free — the hub belongs to another uid
    // (the hub port is machine-wide while this registry is per-user, see
    // docs/platform-support.md), so nothing this process does will move it. Bail
    // and say so. Any other failure is compatible with the port freeing on its
    // own: ESRCH above all, which means the stale hub exited between the probe
    // and the signal — exactly the case where waiting and respawning WORKS. Note
    // it and fall through, or a machine with a hub that died a moment ago is left
    // with no hub at all until the next daemon boots.
    if (health.pid) {
      try {
        kill(health.pid, 'SIGTERM');
      } catch (e) {
        if (e.code === 'EPERM') {
          console.error(`web-chat: the capture hub on port ${port} (pid ${health.pid}) is running an older protocol and cannot be bounced from here: ${e.code}. `
            + 'It most likely belongs to another user on this machine — set WEB_CHAT_HUB_PORT to a port of your own.');
          return null;
        }
        console.error(`web-chat: could not signal the stale capture hub on port ${port} (pid ${health.pid}): ${e.code || e.message} — waiting for the port to free anyway.`);
      }
    }
