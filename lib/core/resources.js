...
function resourceRegistry({ name = 'resource', tiers = [], builtins = [], load, file, write } = {}) {
  ...
  // Union across every tier dir + builtins, each entry tagged with its `tier`,
  // DEDUPED BY NAME so list() agrees with get(). Reads dirs fresh per call.
  // `load` returning null filters non-entries (wrong file type, malformed,
  // missing sidecar) uniformly — a bad entry is skipped, never fatal.
  //
  // Dedup matters because these lists are read by an agent to decide what to
  // use: an undeduped list showed `form-renderer` twice, identically, with no
  // signal which one `use_component` would actually resolve to. Tiers are walked
  // most-specific-first (the same order get() uses), so the first occurrence of a
  // name wins and later tiers are recorded as `shadowed` rather than dropped
  // silently — provenance stays inspectable without the duplicate row.
  function list() {
    const byName = new Map();
    const push = (record, tier) => {
      const key = record && record.name;
      if (key == null) return;
      const existing = byName.get(key);
      if (existing) { (existing.shadows || (existing.shadows = [])).push(tier); return; }
      byName.set(key, { ...record, tier });
    };
    for (const t of tiers) {
      if (!t.dir) continue;
      let names;
      ...
      for (const entry of names) {
        ...
        if (record != null) push(record, t.tier);
      }
    }
    for (const b of builtins) push(b, 'builtin');
