...
// A BIND address, normalised to the form Host headers (and LOCAL_HOSTNAMES) use.
// `server.listen` takes an IPv6 literal BARE — `WEB_CHAT_HOST=2001:db8::5` is
// the spelling that actually binds — while a Host header must bracket it. So the
// raw bind value cannot be handed to hostnameOf, which would split `2001:db8::5`
// at its first colon and compare the string `2001`, refusing the user's own
// bound address with 421. Two or more colons and no bracket is unambiguously a
// bare v6 literal: a `host:port` pair has exactly one.
function bindHostname(listenHost) {
  const h = typeof listenHost === 'string' ? listenHost.trim().toLowerCase() : '';
  if (!h) return null;
  if (!h.startsWith('[') && h.split(':').length > 2) return `[${h}]`;
  return hostnameOf(h);
}
...
function isLocalHost(hostValue, listenHost = LISTEN_HOST) {
  ...
  const bound = bindHostname(listenHost);
...
// What the /preview/node document is allowed to reach.
//
// That document is the graph viewer's thumbnails and its glance card: one
// per visible node, redrawn every time the graph is drawn, each one executing
// the pane scripts of a node the user is NOT on. It is served from the daemon's
// own origin, so without a policy those historical scripts could `fetch`
// /api/render, /api/store or /api/packs/install against LIVE state, unattended
// — which the route's own header comment claimed they could not.
//
// `connect-src 'none'` is the load-bearing directive; the rest is a floor
// (`default-src 'none'`) with the two holes the document genuinely needs:
//
//   script-src 'unsafe-inline' 'unsafe-eval' — the doc inlines the mount runtime
//     and its scripts, and the runtime executes pane code through `new Function`
//     (public/mount-runtime.js). Without 'unsafe-eval' every pane in every
//     thumbnail renders blank.
//   style-src 'unsafe-inline'  — the doc inlines its <style>, and the theme's
//     raw-CSS escape hatch is baked into it.
//   img-src data: blob:       — inline images survive; a pane that pulls an
//     image off the network does not, in the preview only.
//
// This does NOT make the live surface's panes any more constrained — that
// exposure is a standing accepted decision (lib/server/routes/packs.js). What it
// closes is the unattended REPLAY of old pane code against current state.
const PREVIEW_CSP = [
  "default-src 'none'",
...
module.exports = {
  ...
  SHUTDOWN_HEADER, SHUTDOWN_HEADER_VALUE, PREVIEW_CSP,
};
