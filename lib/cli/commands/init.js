...
// `npm i -g` would REPLACE a linked dev checkout with a published build, so the
// update prompt is never offered on one. The package root carrying a .git dir is
// the honest test: that is a working copy, not an installed tarball.
function isDevCheckout() {
  try { return fs.existsSync(path.join(PACKAGE_ROOT, '.git')); } catch { return false; }
}
...
function printReport(state, log) {
  ...
  if (state.surfaces.length) {
    ...
    for (const s of state.surfaces) {
      const flag = s.mine ? '  ←' : '';
      const health = s.reachable ? '' : (s.live ? '  (not answering)' : '  (dead — registry entry is stale)');
...
// lib/driver. /use stores the mount with NO owner, so `deriveRouting` gives it
// 'auto' (the activity layer the tour is teaching stays on) and Claude can clear
// it later with no force:true. A driver render would stamp `service:init`, which
...
async function freshMode(ctx) {
  ...
  if (nodeMajor < 18) {
    log(`  ✗ Node ${process.versions.node} — web-chat needs Node 18 or newer. Upgrade, then run this again.`);
    process.exitCode = 1;
...
async function existingMode(ctx) {
  ...
  if (!rows.length) {
    ...
    for (const r of rows) {
      ...
      const health = r.reachable ? '' : (r.live ? '  (not answering)' : '  (dead — registry entry is stale)');
    ...
    await offer(
      ...
      async () => {
        for (const r of stale) {
          try { portfiles.deletePortfile('server', { root: r.root }); } catch {}
        }
        log(`  cleared.`);
      },
    );
  ...
  if (latest && latest.updateAvailable) {
    if (isDevCheckout()) {
      log(`v${latest.latest} is out, but you're on a linked dev checkout; \`npm i -g\` would replace it.`);
      log(`  Update with \`git pull\` in ${PACKAGE_ROOT} instead.`);
    } else {
      log(`v${latest.latest} is available (you are on v${version}).`);
      log(`  \`claude-web-chat update\` currently runs: npm i -g git+https://github.com/Brynhild-CHale/claude-web-chat.git`);
      log(`  Distribution is moving to GitHub Releases; this command has not caught up yet.`);
      // Default No, and never taken by --yes: this rewrites a global install.
...
async function init(args = [], opts = {}) {
  ...
  const deps = {
    ...
    collectRows: opts.collectRows || require('./ls').collectRows,
