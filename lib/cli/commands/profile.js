...
// HTTP status, so a daemon predating this route answered 404 with an HTML body
// and this printed "✓ reloaded undefined user profile(s) — live now, no restart"
// having reached nothing. post() throws an HttpError carrying that status and
// body, so a failure reads as one. noSpawn, because a hot reload must never
// resurrect a daemon the user closed — the no-daemon case is the line above.
async function reload() {
  ...
  let body;
  try { body = await client.post('/api/profiles/reload', {}, { port, root, noSpawn: true, timeout: 5000 }); }
  catch (e) { console.error(`reload failed: ${e.message}`); process.exit(1); }
  console.log(`✓ reloaded ${(body && body.count) || 0} user profile(s) — live now, no restart`);
}
