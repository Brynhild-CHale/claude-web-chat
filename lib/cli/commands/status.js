...
async function status(args = [], opts = {}) {
  ...
  if (fs.existsSync(p.dir)) {
    if (reg.managedError) {
      ...
      // An unmerged `.new` beside a settled file counts here: `status` reports
      // state, and "you have an offer open" is state. It is not "needs refresh"
      // — install would do nothing for it — so it goes through the conflict
      // summary, which names the by-hand step, rather than the refresh line.
      const unmerged = results.filter(r => r.action !== 'conflict' && r.pending);
      if (conflicts.length || unmerged.length) {
