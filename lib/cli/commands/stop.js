...
// LOCK_DRAIN_TIMEOUT_MS (30s waiting out a live turn) + INFLIGHT_DRAIN (5s), so
// anything under ~35s would time out on a daemon that is behaving correctly —
// and then SIGTERM it mid-drain, aborting the very snapshot we asked for
// (gracefulShutdown is re-entrant-guarded: the signal handler would find
// shuttingDown already true and go straight to process.exit).
