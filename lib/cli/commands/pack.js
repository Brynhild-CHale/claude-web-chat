...
function list(opts) {
  ...
  // so a marker surviving means the process died before it could; re-running the
  // install is the whole recovery.
  if ((pending || []).length) {
    console.log('');
    console.log('  interrupted installs (started, never finished — nothing is registered):');
    for (const p of pending) {
      console.log(`    ${p.name}${p.version ? ` ${p.version}` : ''}   ${tierWord(p.tier)}   started ${p.started_at || '?'}`);
