...
function list(opts) {
  ...
  for (const p of installed) {
    // verifyPack counts a REFUSED unit as drift — its record disagrees with
    // anything this machine could have written — so labelling every drift
    // "locally edited" told the user their own edits were the problem for a
    // record that never validated. `pack info` and `pack remove` already
    // distinguish the two; this line is the one that did not, and it has the
    // unit states to do it with.
    const refused = opts.verify && (p.unit_states || []).some((u) => u.state === 'refused');
    const drift = !opts.verify ? ''
      : refused ? '  · record refused'
        : p.drift ? '  · locally edited' : '  · unmodified';
