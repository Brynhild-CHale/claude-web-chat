...
// ── list / info ─────────────────────────────────────────────────────────────
function list(opts) {
  const { packs: installed, quarantined } = packs.listInstalled({ root: root(), verify: opts.verify === true });
  console.log('');
...
function info(name) {
  ...
  if (found) {
    ...
    console.log('');
    for (const u of v.units) console.log(`    ${u.kind.padEnd(9)} ${u.name.padEnd(24)} ${u.state}`);
    console.log('');
    if (v.drift) console.log(`  locally edited — \`pack remove ${name}\` will keep the edited units unless you pass --force\n`);
    return;
  }
