...
async function doctor(args = [], opts = {}) {
  ...
  if (state.mcp.resolvable) {
    ...
    if (dryRun) {
      ...
      // Run it from the root doctor resolved: `--scope local` registers the
      // directory `claude` is invoked in, so from a subdirectory this would
      // register a project doctor never examined.
