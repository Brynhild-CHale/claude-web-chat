...
Commands:
  ...
  trust [name] [--deny] Approve (or refuse) a component's host-side service.js.
                        With no name, lists what is waiting. This is the ONLY way
                        to approve one: the surface cannot grant it, because a
                        component's own pane script runs in that page.
  ...
  ls [--reap]           List every web-chat surface running on this machine — which
                        project each one is, and on what port. --reap stops the
                        ones that aren't this project and clears stale entries.
