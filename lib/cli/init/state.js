...
// ships". `up-to-date`/`kept-edited` are settled states and never drift.
const DRIFT_ACTIONS = new Set(['created', 'updated', 'differs', 'conflict']);

// A `pending` row (an unmerged `<dest>.new` beside the file) DOES count here,
// even though its action is the settled `kept-edited`. This state feeds `init`'s
// orientation report, whose whole job is "here is the unfinished business in
// this project" — an offer the user has neither merged nor dismissed is exactly
// that, and the sidecar path is the only thing they need to act on it.
function isDrift(r) {
