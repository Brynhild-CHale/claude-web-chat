...
// dead process" rule.
//
// Nothing here signals MID-DRAIN, and nothing here signals a pid that has not
// answered /api/health as the pid the registry listed. The per-row budget below
// is shorter than the daemon's own worst-case drain (a live turn can hold
// gracefulShutdown for 30s), so an ack that has not turned into an exit yet
// means "still working", not "wedged" — which is why every stop() from here
// passes `signalAfterAck:false`. Interrupting a drain that is going fine buys
// nothing and costs a wait we then have to repeat for the next row. (It used to
// cost more: a SIGTERM mid-drain re-entered the boolean-guarded
// gracefulShutdown, returned instantly and process.exit()ed before writeDraft —
// destroying the snapshot the ask was for and leaving the portfile behind. A
// second trigger awaits the in-flight shutdown now, but the reasoning for not
// signalling here is unchanged.)
//
// What that flag does NOT cover, and the reason this header no longer claims
// "never signals": signalAfterAck gates the ACKNOWLEDGED branch alone. A daemon
// that answered /api/health as our pid and then failed to acknowledge the
// shutdown request — a request error, a non-ok body, a build too old to have the
// route — still falls through to stop()'s SIGTERM. That one is draft-safe
// (nothing is draining, and the handler runs a full gracefulShutdown) and is
// exactly the escalation `stop` itself would make for the same row.
//
...
async function reap(rows, { here = null, log = console.log, ackWaitMs, signalWaitMs } = {}) {
  ...
  for (const r of rows) {
    ...
    // A row with no `root`. registerInstance always writes one, so this is a
    // hand-edited or legacy record — and everything below addresses a surface BY
    // its root, where a missing one resolves to process.cwd(): release() would
    // reach for the portfile of the project the user is standing in (and throw
    // out of instanceId(null) into its own catch, so the entry survived and `ls`
    // printed it forever with a "--reap clears" hint that never did). The entry
    // carries its own id, so a dead one is still removable by that; a live one is
    // left alone, because we have no idea which surface it describes.
    if (!r.root) {
      if (!r.pid_alive && registry.deregisterById(r.id, { pid: null })) { cleared++; continue; }
      skipped.push({ row: r, reason: 'no-root' });
      log(`  ${nameOf(r)} — this registry entry names no project root, so there is nothing to address. Remove it by hand from ${registry.registryPath()}`);
      continue;
    }
