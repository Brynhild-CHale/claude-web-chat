...
// a daemon that needs longer is reported, not killed harder.
...
async function stopRow(row, {
  ...
  return stop([], { root: row.root, log, ackWaitMs, signalWaitMs });
}
...
async function reap(rows, { here = null, log = console.log, ackWaitMs, signalWaitMs } = {}) {
  ...
  for (const r of rows) {
    ...
    skipped.push({ row: r, reason: res.reason || 'failed' });
