...
const { isLocalOrigin } = require('../core/cors');

const SHUTDOWN_GRACE_MS = 10_000;
const HEARTBEAT_INTERVAL_MS = 30_000;
const HEARTBEAT_TIMEOUT_MS = 5_000;

function attachWebSocket(httpServer, { state, graph, paths, bus, triggerShutdown, projectName, onViewersChanged, onServiceDecision }) {
  ...
  const wss = new WebSocketServer({
    server: httpServer,
    path: '/ws',
    verifyClient: ({ origin }) => !origin || isLocalOrigin(origin),
  });
  ...
  wss.on('connection', (ws) => {
    ...
    ws.on('message', (raw) => {
      ...
      } else if (msg.type === 'service:decision') {
        // Consent for running a component's host service. Deliberately its OWN
        // frame type rather than a store key: any pane can write the store, so a
        // component could otherwise approve its own service.js. The supervisor
        // validates the nonce against its outstanding prompt and ignores
        // anything that doesn't match.
        if (typeof onServiceDecision === 'function') {
          try { onServiceDecision({ hash: msg.hash, decision: msg.decision, nonce: msg.nonce }); } catch {}
        }
