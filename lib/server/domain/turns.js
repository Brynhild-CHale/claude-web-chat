...
// (SNAPSHOT_FIELDS per mount + store/comments/captures) — never queue/pendingWake,
// which are live-only.
...
// branch-here to decide whether uncommitted work needs auto-preserving before
...
// The pre-guard shared by set-active / wipe / new-graph / branch-here (re-aim
...
// (nothing was ever committed), so the steal auto-preserves first, by exactly
// the mechanism branch-on-edit uses: commit the dirty live surface as its own
// node on the current commit point, then let the re-aim proceed. Authored
// 'claude' because the abandoned turn's work is what is being saved.
function guardReaim(graph, bus, { draftPath } = {}) {
  ...
  return { blocked: false, stole_stale_lock: true, ...(preserved ? { preserved } : {}) };
}
...
function accumulateFolded(graph, { author = 'claude', kind = 'turn', message = '', summary = '' } = {}) {
...
// commit point (set-active / branch-here-with-nothing-to-preserve / new-graph):
...
function skipTurn(graph, bus, { author = 'claude', summary } = {}) {
...
function commitNode(graph, bus, {
  ...
  // branch-here's auto-preserve alike. That is what makes branch-here correct
