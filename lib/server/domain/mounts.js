...
const POLICIES = {
  default: { force: false, emitEvent: true, bypassLock: false, carryForm: true },
  // The tab-stream extension's capture panes. Four of these five deviations are
  // artefacts of the hand-copy rather than intent and are flipped — with the
  // frozen wire golden regenerated — in their own commit.
  capture: { force: true, emitEvent: false, bypassLock: true, carryForm: false },
};
...
function setMount(state, bus, {
  ...
  if (!pol.bypassLock && existing && existing.pane_state && existing.pane_state.locked) {
  ...
  const form_state = (pol.carryForm && existing && !(params && params.form_reset)) ? existing.form_state : undefined;
  ...
  const nextTheme = theme !== undefined ? theme : (existing ? existing.theme : undefined);
  ...
  bus.emit({
    event: pol.emitEvent ? { kind: 'render', id, target, bytes: String(html || '').length, source: who, component } : null,
