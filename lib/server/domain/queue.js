...
function revertActivity(state, bus, item) {
  ...
  bus.emit({
    event: { kind: 'render', id: mountId, target: mount.target, bytes: (mount.html || '').length, source: 'queue-revert' },
    ws: {
      type: 'render', id: mountId, html: mount.html, target: mount.target, params: mount.params,
      component: mount.component, pane_state: mount.pane_state, form_state: mount.form_state, theme: mount.theme,
    },
  });
  return true;
}
