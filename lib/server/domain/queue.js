...
function emitWake(bus, batch, { reason = 'push', source = 'queue', note } = {}) {
...
function drainPending(state, bus) {
  const park = state.pendingWake;
  if (!park) return null;
  state.pendingWake = null;
  return emitWake(bus, park.batch, { reason: park.reason, source: park.source, note: park.note });
}
...
function flush(state, bus, { reason = 'push', source = 'queue', note } = {}) {
  ...
  const wake = emitWake(bus, batch, { reason, source, note });
  return { batch, wake, parked: null };
}

module.exports = { enqueue, list, remove, removeByComment, refreshComment, setStaged, flush, emitWake, parkWake, drainPending, consumePending, revertPane, revertArtifact, isStaged, MAX_QUEUE };
