...
const { writePortfile, deletePortfile, probeReachable } = require('../core/portfiles');
const { LISTEN_HOST, warnIfExposed } = require('../core/cors');
...
function createServer({ root = process.cwd(), port = 'auto' } = {}) {
  ...
  async function gracefulShutdown() {
    ...
    // On Node 18, server.close() waits for every existing connection INCLUDING
    // idle keep-alive sockets — and lib/client's http.globalAgent keeps sockets
    // pooled (globalAgent.keepAlive has defaulted true since Node 19, and any
    // long-lived caller can pool one on 18 too). One idle pooled socket is
    // enough to stall close() past the caller's wait, so the portfile never gets
    // deleted and a clean shutdown reads as a failure. Node 19 made close() hang
    // up on idle connections itself; this call is what gives Node 18 (the
    // engines floor) the same behaviour. Guarded — added in Node 18.2.
    if (typeof server.closeIdleConnections === 'function') {
      try { server.closeIdleConnections(); } catch {}
    }

    await new Promise((resolve) => server.close(() => resolve()));
    // Only if the record still names THIS process: a second daemon that came
    // up on the same root owns the file now, and its note must stay on the door.
    deletePortfile('server', { root, pid: process.pid });
