...
// `Object.hasOwn`, never `in`: state.store is a plain object, so `'toString' in
// {}` is true and a store key named after an Object.prototype member — toString,
// constructor, valueOf, hasOwnProperty — was skipped by the added loop AND not
// iterated by the removed loop (it is not an own key of the other side), so it
// disappeared from the diff rather than being reported wrongly.
function diffStore(aStore, bStore) {
  const added = {}, removed = {}, changed = {}, unchanged = [];
  for (const k of Object.keys(bStore)) if (!Object.hasOwn(aStore, k)) added[k] = truncVal(bStore[k]);
  for (const k of Object.keys(aStore)) {
    if (!Object.hasOwn(bStore, k)) { removed[k] = truncVal(aStore[k]); continue; }
...
function diffTheme(aTheme, bTheme, opts) {
  ...
  // Same rule as diffStore above. Every token name is --wc- prefixed today, so
  // nothing can collide — but the loop must not depend on that staying true.
  for (const k of Object.keys(bt)) if (!Object.hasOwn(at, k)) tokens.added[k] = bt[k];
  for (const k of Object.keys(at)) {
    if (!Object.hasOwn(bt, k)) tokens.removed[k] = at[k];
