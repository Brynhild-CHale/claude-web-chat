...
function createGraph({ paths, state }) {
  ...
  graph.load = () => {
    ...
    for (const f of files) {
      ...
      console.error(`[web-chat] skipping unreadable graph node ${f}: ${why}`);
    }
    ...
    if (metaRead.ok) {
      ...
      console.error(`[web-chat] graph/_meta.json is unreadable (${why}); recovering the active node from the latest commit`);
    }
