...
function createServiceSupervisor({ state, graph, paths, bus, getPort, getViewers, log = () => {} }) {
  // mountId -> { child, name, hash, paramsFp, status, params, servicePath }
  ...
  //
  // The key is NOT the service.js hash alone. Two things besides the source
  // decide what a service actually does:
  ...
  // So consent is recorded per (project root, service source, params shape).
  ...
  // Stable fingerprint of the params a service is being spawned with. Key order
  // is normalised so an equivalent object doesn't read as a different request.
  ...
  function hasDecision(hash, paramsFp) {
    return Boolean(readTrusted()[trustKey(hash, paths.root, paramsFp)]);
  }

  function isTrusted(hash, paramsFp) {
    if (!hash) return false;
    const rec = readTrusted()[trustKey(hash, paths.root, paramsFp)];
  ...
  function computeDesired({ ignoreViewers = false } = {}) {
    ...
    for (const [mountId, m] of state.mounts) {
      ...
      out.set(mountId, {
        name: m.component, params: m.params, servicePath: info.servicePath, hash: info.hash,
        paramsFp: paramsFingerprint(m.params),
      });
    }
    return out;
  }
  ...
  function reconcile(reason) {
    ...
    //    (service.js hash, params fingerprint) — the same pair consent is keyed
    //    to. Params are not decoration: `file-editor`'s `unfenced:true` is what
    ...
    for (const [mountId, entry] of [...children]) {
      ...
      if (!d || d.hash !== entry.hash || d.paramsFp !== entry.paramsFp) stop(mountId);
    }
  ...
  function prune() {
    ...
    const live = new Set([...onSurface.values()].map((d) => trustKey(d.hash, paths.root, d.paramsFp)));
    for (const [key, p] of [...prompted]) {
      if (!live.has(key)) { prompted.delete(key); clearTrustPrompt(p.hash); }
    }
  ...
  function ensureStarted(mountId, d) {
    if (isTrusted(d.hash, d.paramsFp)) { spawn(mountId, d); return; }
    // Refused earlier: stay stopped, and stay quiet.
    if (hasDecision(d.hash, d.paramsFp)) return;
  ...
  function spawn(mountId, d) {
    ...
    const entry = { child, name: d.name, hash: d.hash, paramsFp: d.paramsFp, status: 'starting', params: d.params, servicePath: d.servicePath };
  ...
  function surfaceTrustPrompt(d) {
    const key = trustKey(d.hash, paths.root, d.paramsFp);
    if (prompted.has(key)) return;
    prompted.set(key, {
      name: d.name, hash: d.hash, params: d.params || {}, paramsFp: d.paramsFp,
    ...
    bus.emit({ ws: { type: 'service:trust', name: d.name, hash: d.hash, command: `claude-web-chat trust ${d.name}` } });
  }
  ...
  function reissuePrompts() {
    for (const [, p] of prompted) {
      bus.emit({ ws: { type: 'service:trust', name: p.name, hash: p.hash, command: `claude-web-chat trust ${p.name}` } });
    }
  }
  ...
  function pendingTrust() {
    return [...prompted.values()].map((p) => ({
      ...
      root: p.root, params_fp: p.paramsFp,
      key: trustKey(p.hash, p.root, p.paramsFp),
    }));
  }
  ...
  function refreshTrust() {
    for (const [key, p] of [...prompted]) {
      ...
      if (hasDecision(p.hash, p.paramsFp)) { prompted.delete(key); clearTrustPrompt(p.hash); }
    }
