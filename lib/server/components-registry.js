...
const { assertComponentName } = require('../core/names');
  ...
  return resourceRegistry({
    ...
    load: (entryDir, { name }) => {
      ...
      try {
        ...
        // The DIRECTORY name is the identity, not meta.json's `name`.
        //
        // list() used to key by meta.name while get() has always resolved by
        // directory, so a component whose meta.json disagreed with its folder
        // was visible in the drawer and the palette under a name that
        // use_component could not resolve — listed but unspawnable. The
        // mismatch is still worth surfacing (a pack author almost certainly
        // meant them to agree), so it rides along as `meta_name` for the UI to
        // flag rather than being silently preferred.
        return {
          name,
          meta_name: meta.name && meta.name !== name ? meta.name : undefined,
          description: meta.description,
          params_schema: meta.params_schema,
          builtin: meta.builtin === true ? true : undefined,
          has_seed,
          has_service,
        };
      } catch {
    ...
    write: (dir, name, { source, description, params_schema, seed, service }) => {
      // resourceRegistry.save() states that the caller owns name validation and
      // reserved-name policy. It is right that the type-specific MESSAGE belongs
      // to the route — but this is the function that turns a name into a
      // directory and writes a meta.json without a `builtin` marker, so the
      // policy is re-asserted here too. Any future caller of registry.save gets
      // the refusal without having to remember to ask for it.
