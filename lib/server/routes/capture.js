...
// Pane rendering lives in lib/capture/pane.js — the route calls it, the CLI's
// `profile dry-run` calls the same copy, and nothing imports a route module.
const {
  feedbackCard, selectionCard, renderProfilePane, renderSimplifiedPane,
} = require('../../capture/pane');
const { projectPaths } = require('../../core/paths');
const { setCors } = require('../../core/cors');
const { setMount, removeMount } = require('../domain/mounts');
...
function mountCaptureRoutes(app, { state, graph, paths, bus }) {
  ...
  app.post('/api/capture', (req, res) => {
    ...
    // What happened to the PANE, which is not what happened to the capture. The
    // record is written and `tab_capture` is bumped either way, but setMount
    // soft-rejects over a user-LOCKED pane (decision D1) — and the route used to
    // drop that envelope on the floor and answer a bare {ok:true}, so the
    // extension showed a successful capture over a pane that never updated with
    // nothing, anywhere, saying why. Additive on the wire; a caller that does not
    // know the field ignores it.
    let pane = 'not-requested';
    const notePane = (r) => { pane = r && r.ok ? 'rendered' : r; };
    ...
    if (render && isSelection) {
      ...
      // The shared mount-set (lib/server/domain/mounts) under the `capture`
      // policy. The empty pane_state patch is the spread copy this pane has
      // always written: the id carries the capture id, so there is never an
      // existing pane to carry state from.
      notePane(setMount(state, bus, {
        id: mountId,
        html: selectionCard(record, bodyHtml),
        target: 'main',
        params: { title: title2, modes: false },
        owner: CAPTURE_OWNER,
        policy: 'capture',
        pane_state_patch: {},
      }));
    ...
    if (render && !isSelection) {
      ...
      removeMount(state, bus, { id: LEGACY_MOUNT_ID, source: CAPTURE_OWNER });

      // The shared mount-set under the `capture` policy. `mode` is the one
      // pane_state PATCH in the tree — the user's reduced/expanded choice is
      // carried forward and re-asserted on every re-capture.
      notePane(setMount(state, bus, {
        id: mountId,
        html: html2,
        target: 'main',
        params: { title: title2, modes: hasModes, mode },
        owner: CAPTURE_OWNER,
        policy: 'capture',
        pane_state_patch: { mode },
      }));
    }

    res.json({ ok: true, capture_id: id, seq, profile: usedProfile, distilled, bytes_raw: record.bytes_raw, raw_ref: rawRel, fell_back_from, error, pane });
  });
...
module.exports = { mountCaptureRoutes };
