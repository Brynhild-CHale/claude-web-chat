...
function mountEventRoutes(app, { state, bus, retain, release, sseClients }) {
  ...
  app.get('/api/events/stream', (req, res) => {
    ...
    if (isWakeConsumer) state.wakeConsumers++;
    let closed = false;
    ...
    if (isWakeConsumer && state.pendingWake) queue.drainPending(state, bus);
