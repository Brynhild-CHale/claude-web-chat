...
// Is a directory name under versions/ a version at all? The store is written by
// this program alone, so anything whose numeric core does not parse got there by
// accident — an interrupted download's staging directory, a hand-made folder —
// and must never be listed, activated, rolled back to, or protected from
// pruning. The shape is compareVersions' `numeric` case, spelled strictly (that
// one parseInts, so `1x` would pass): a dotted run of digits with an optional
// `v` and an optional `-prerelease`. `0.7.0` and `0.8.0-rc1` pass;
// `wc-release-Ab12Cd` — which the comparator's lexical fallback used to sort
// ABOVE every real release — does not.
function isVersionName(name) {
  return /^v?\d+(?:\.\d+)*(?:-[0-9A-Za-z.-]+)?$/.test(String(name));
}
