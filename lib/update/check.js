...
// Compare two dotted numeric versions. Returns >0 if a is newer, 0 if equal,
// <0 if a is older. Non-numeric tails (a `-rc.1` suffix) are ignored for
// ordering but make an otherwise-equal version sort OLDER, so a prerelease never
// advertises itself over the final release of the same number.
...
// GitHub's API requires a User-Agent and 404s (rather than erroring) when a repo
// has no published release yet — both handled as a silent "no update known".
function fetchLatest(timeoutMs = 2500) {
  return new Promise((resolve, reject) => {
    const req = https.get(RELEASES_API, {
      headers: { 'User-Agent': 'claude-web-chat', Accept: 'application/vnd.github+json' },
    }, (res) => {
      let body = '';
      ...
      res.on('end', () => {
        ...
        try {
          ...
          if (json.draft) return resolve(null);
          resolve(json.tag_name ? String(json.tag_name).replace(/^v/, '') : null);
        } catch (e) {
          reject(e);
        }
      });
...
module.exports = { check, resolveLatest, clearCache, cachePath, compareVersions, RELEASES_PAGE };
