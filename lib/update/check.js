...
// The programmatic check: resolve the latest published version (throttled +
// cached like check() below) and report whether it differs from the running one.
// `force:true` bypasses the 24h throttle (the surface's "check now"). Never
// throws — a failed fetch degrades to the last cached `latest` (or null). Returns
// { current, latest, updateAvailable, checkedAt }. The stderr `check()` and the
// server's GET /api/version share this one implementation so "is an update
// available" is decided in exactly one place.
async function resolveLatest({ currentVersion, force = false } = {}) {
  ...
  return {
    ...
    latest,
    // Any difference counts (matches the stderr check's long-standing behavior);
    // there's no semver dep, and the remote never publishes an older build.
...
module.exports = { check, resolveLatest, clearCache, cachePath };
