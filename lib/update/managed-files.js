...
const MANAGED_FILES = [
  ...
  { tpl: 'skills/respond-to-comment/SKILL.md', dest: '.claude/skills/respond-to-comment/SKILL.md' },
];
...
// The hook bin, as an absolute path. Same reasoning as ensureMcpRegistration
// below: hooks run as a shell command, so a bare `claude-web-chat-hook` resolves
// only when the package is on PATH (a global install / `npm link`). In a plain
// checkout — or a session whose PATH lacks npm's global bin — it exits 127 and
// the turn lifecycle silently never runs: no lock, no commit, an empty graph.
// `node "<abs path>"` always runs.
const hookBin = path.join(__dirname, '..', '..', 'bin', 'claude-web-chat-hook.js');

// Rewrite a bare `claude-web-chat-hook <sub>` into `node "<abs>" <sub>`.
// Idempotent: an already-resolved command has no bare token left to match.
function resolveHookCommand(cmd) {
  return String(cmd).replace(/(^|\s)claude-web-chat-hook(?=\s|$)/, `$1node "${hookBin}"`);
}
...
function ensureMcpRegistration(root) {
  ...
  // `${CLAUDE_PLUGIN_ROOT}/bin/...` (see .claude-plugin/plugin.json). When we're
  // actually running under that packaging (the env var is set), preserve it —
  // rewriting it to this machine's absolute path would de-portabilize a file the
  // plugin owns. Outside plugin packaging the placeholder can't resolve, so the
  // absolute-path rewrite below stays the right fix.
  const portable = hadEntry && JSON.stringify(existing.args || []).includes('${CLAUDE_PLUGIN_ROOT}');
  if (portable && process.env.CLAUDE_PLUGIN_ROOT) {
    const kept = { ...existing };
