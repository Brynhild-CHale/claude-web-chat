...
// Destroy a write stream and wait until it has actually CLOSED.
//
// fs.createWriteStream opens its fd on the libuv threadpool, and destroy() does
// not cancel a pending open — it waits for it, then closes. So a destroy issued
// before the open lands still CREATES the file, moments later. Unlinking
// straight away therefore loses the race: the unlink comes back ENOENT (and is
// swallowed), and the open then strands a 0-byte `.part` in the version cache
// forever. Reproduced deterministically by keeping the threadpool busy while the
// overflow fires.
function closeStream(stream) {
  if (stream.closed) return Promise.resolve();
  return new Promise((resolve) => {
    // The chunk that lost the race to the destroy reports "write after end";
    // there is nobody left to tell, and it must not become an unhandled error.
    stream.on('error', () => {});
...
// Debris from an update that was killed mid-download. The `finally` below cleans
// up every ordinary failure, but nothing runs on SIGINT (no handler exists in the
// CLI), so a Ctrl-C at the wrong second strands a whole tarball. Only our own
// prefix, and only entries old enough that they cannot belong to an update
// running right now.
...
function sweepStaleStaging(dir, now = Date.now()) {
  ...
  for (const name of names) {
    if (!name.startsWith('wc-release-')) continue;
    const full = path.join(dir, name);
    try {
      if (now - fs.statSync(full).mtimeMs < STALE_STAGING_MS) continue;
      fs.rmSync(full, { recursive: true, force: true });
    } catch { /* someone else's, or already gone — never worth failing an update */ }
  }
}
...
async function fetchAndUnpack({ release, versionDir, log = () => {}, tmpDir } = {}) {
  ...
  const stageRoot = tmpDir || os.tmpdir();
  sweepStaleStaging(stageRoot);
  const work = fs.mkdtempSync(path.join(stageRoot, 'wc-release-'));
  try {
