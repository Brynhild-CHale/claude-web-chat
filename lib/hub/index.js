...
const { setCors, LISTEN_HOST, requireLocalHost } = require('../core/cors');
...
function createHub({ port = hubPort() } = {}) {
  const app = express();
  // The Host gate first (lib/core/cors). The hub is the easiest rebinding target
  // in the product because its port is FIXED (5170) rather than walked, so an
  // attacker can name it in advance; /api/instances lists every running project
  // and /api/capture forwards into one of them.
  ...
  function shutdown(code) {
    ...
    // Only if the entry still names THIS process. A hub that lost the fixed-port
    // race and is on its way out must not take the winner's registration with it.
