...
const { BUILTIN_COMPONENTS, COMPONENT_NAME_RE, assertComponentName } = require('../core/names');
const { managedSkillNames } = require('../update/managed-files');

const MANIFEST_FILE = 'web-chat-pack.json';
const SKILL_FILE = 'SKILL.md';
// One grammar, declared in lib/core/names.js. Re-exported under the name this
// module has always used for it so callers and tests do not have to care.
const NAME_RE = COMPONENT_NAME_RE;
...
function satisfies(version, range) {
  ...
  // Shapes this grammar does not implement, but whose numeric core the clause
  // regex below matches happily — which is worse than not matching at all: an
  // x-range (`1.x`, `1.2.*`) becomes an EXACT pin on the numeric prefix, a
  // disjunction (`^1 || ^2`) is read as a conjunction that nothing can satisfy,
  // and a hyphen range (`1.2.0 - 2.0.0`) as two exact pins. Every one of those
  // ends in `this pack needs web-chat 1, this is 1.5.0` and a refusal — exactly
  // what the promise above says must not happen. Unknown means unknown.
  if (/[xX*]|\|\||\s-\s/.test(r)) return { ok: true };
...
// Reserved names, in one place each. The builtin components come from
// lib/core/names; the skill names are derived from the managed-file list (which
// core must not import, so they stay here).
function reservedComponentNames() { return BUILTIN_COMPONENTS.slice(); }
...
//
// The grammar check is lib/core/names.assertComponentName; `reserved: false`
// because a PACK name is checked against reserved SKILL names (validateManifest
// above), never against the builtin component list — sharing a name with a
// component is not what makes a pack name unsafe.
function assertSafeName(name, what = 'pack name') {
  return assertComponentName(name, { what, reserved: false });
}
