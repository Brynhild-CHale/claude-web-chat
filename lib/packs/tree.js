...
function verifyPack(pack, { root, tier } = {}) {
  ...
  for (const unit of pack.units || []) {
    // The record is a FILE on disk (.web-chat/packs.json), and `unit.name` out
    // of it is joined straight into a destination directory that removeUnits
    // then unlinks inside. Everything that writes a record validates the name
    // first — but "everything that writes it" is not the same guarantee as
    // "whatever is in the file", so the name is re-checked at read time against
    // the one grammar. A unit that fails is reported and SKIPPED rather than
    // thrown on: `pack remove` over a damaged record should degrade to leaving
    // that unit alone, not crash and strand the rest of the pack.
    if (!isComponentName(unit.name)) {
      drift = true;
      units.push({
        kind: unit.kind,
        name: String(unit.name == null ? '' : unit.name),
        dest: null,
        state: 'unsafe-name',
        files: [],
      });
...
function removeUnits(pack, { root, tier, force = false, dryRun = false } = {}) {
  ...
  for (const unit of verified.units) {
    ...
    if (unit.state === 'unsafe-name') {
      // Nothing is unlinked for a unit whose recorded name is not a plain
      // kebab-case name. keptAny, so the record survives for someone to look at.
      keptAny = true;
      results.push({ dest: label, action: 'skipped (unsafe name)', kind: unit.kind, name: unit.name });
      continue;
    }
