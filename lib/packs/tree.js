...
const { isInside, homeDir } = require('../core/paths');
...
//
// So fence the nearest ancestor that DOES exist. The tail below it is safe by
// construction and not by luck: `unitDir` builds the path by joining a
// NAME_RE-checked name (or, for a theme, a constant) onto the path authority,
// so it contains no `..` to walk back out with — and a directory that is absent
// has nothing under it to unlink anyway. A committed `.web-chat/components ->
// /outside` still exists, so it is still the ancestor that gets resolved, and
// it is still refused.
