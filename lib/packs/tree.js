...
function unitRecordFault(unit, dir, anchor) {
  ...
  // The unit directory itself, against the unforgeable anchor. This is the
  // check that catches a committed symlink anywhere in `.web-chat/...` — a
  // directory that does not exist realpaths to itself and passes, which is
  // right: there is nothing under it to unlink.
  if (!anchor || !isInside(anchor, dir)) {
