...
function stageQuarantine({ stageDir, files, name, root, quarantineRoot, tier, source, plan, manifest }) {
  ...
  const record = {
    name,
    // The PROJECT this tree was staged in. The record is user-tier and the tree
    // is not, so without this half the store cannot tell two projects' packs of
    // the same name apart — see the quarantine note in lib/packs/store.js.
    root,
    tier,
    dir,
    pack_dir: packDir,
...
// they do not.
function promoteQuarantine(root, name) {
  assertSafeName(name);
  const record = findQuarantine(root, name);
  if (!record) {
    const e = new Error(`nothing is quarantined under "${name}" in this project on this machine. If you see a .web-chat/packs/quarantine/${name} directory, it was not staged here — refusing it.`);
    e.userFacing = true;
    throw e;
  }
