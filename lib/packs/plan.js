...
function planInstall({ stageDir, manifest, tier = 'local', root, replace = false } = {}) {
  ...
  //
  // Built from the path authority directly, NOT from lib/server/paths: that
  // adapter calls ensureProjectDirs, so every plan — including the one
  // quarantinePack computes for a tree the user has not approved — created
  // .web-chat/components, services, graph, captures, themes and profiles in the
  // target project. A function whose header says it writes nothing was mkdir'ing
  // six directories, and the quarantine promise "nothing is installed, nothing
  // is registered" was not true.
  let registry = null;
  try {
    registry = componentsRegistry({
      COMPONENTS_DIR: projectPaths(root).components,
      SYSTEM_COMPONENTS_DIR: userPaths().components,
    });
  } catch { registry = null; }
  const units = [];
  const collisions = [];
  ...
  for (const c of manifest.components || []) {
    // Grammar before path. validateManifest already refused a non-kebab name,
    // but quarantine deliberately plans packs that FAILED validation (seeing why
    // is the point of review), so plan() cannot assume validation ran and
    // agreed. Both joins below are the reason: a name carrying a separator
    // aims destDir outside the components directory entirely.
    if (!isComponentName(c.name)) {
      errors.push(`component ${JSON.stringify(String(c.name == null ? '' : c.name))}: names must be kebab-case — refused`);
      continue;
    }
  ...
  for (const th of manifest.themes || []) {
    if (!isComponentName(th.name)) {
      errors.push(`theme ${JSON.stringify(String(th.name == null ? '' : th.name))}: names must be kebab-case — refused`);
      continue;
    }
