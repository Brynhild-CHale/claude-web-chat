...
function installFromStage({ stageDir, source, tier, root, replace = false, actor = 'cli', log = () => {}, via = 'install' }) {
...
async function installPack({ url, ref = null, asset = null, tier = 'local', root, replace = false, actor = 'cli', log = () => {} } = {}) {
  ...
  try {
    return installFromStage({
      stageDir: fetched.stageDir, source: fetched.source,
...
function reviewQuarantine({ name, root, file = null, maxBytes = 512 * 1024 } = {}) {
  assertSafeName(name);
  // `root` is load-bearing, not decoration: a quarantine record is keyed by
  // (project, name), so this reads THIS project's staged tree and never another
  // project's pack that happens to share the name.
  const record = findQuarantine(root, name);
  if (!record) throw refuse(`nothing is quarantined under "${name}" in this project`);
...
function approvePack({ name, root, tier = null, replace = false, actor = 'cli', log = () => {} } = {}) {
  const { record, stageDir } = promoteQuarantine(root, name);
  const out = installFromStage({
    stageDir, source: record.source,
    tier: normTier(tier || record.tier), root, replace, actor, log, via: 'approve',
  });
  removeQuarantineRecord(root, name);
...
function listInstalled({ root, verify = false } = {}) {
  ...
  // This project's staged trees only — another project's are its business, and
  // reporting them here made `present` (an absolute pack_dir) read as ours.
  const quarantined = listQuarantine(root).map((q) => ({
