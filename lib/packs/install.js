...
function installFromStage({ stageDir, files, source, tier, root, replace = false, actor = 'cli', log = () => {}, via = 'install' }) {
  ...
  putPending(root, tier, {
    ...
    started_at: new Date().toISOString(), pid: process.pid, source,
    units: plan.units.map((u) => `${u.kind}:${u.name}`),
  });
  ...
  try {
    ...
    appendAudit(root, {
      ...
      rolled_back: true, ...(failures.length ? { rollback_errors: failures } : {}),
    });
    // The rollback's own failures cannot be silent — the audit log is allowed to
    // swallow its write, so they ride the error the caller is about to see.
    if (failures.length) e.rollbackErrors = failures;
    throw e;
  }
  journal.commit();
  ...
  appendAudit(root, {
    ...
    ...(pruned.length ? { removed: pruned.map((r) => `${r.kind}:${r.name}`) } : {}),
  });
