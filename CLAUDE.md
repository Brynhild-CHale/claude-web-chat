...
| decide who may reach this server (bind host, `Host` gate, WS `Origin` gate, extension CORS, the preview document's CSP) / escape HTML / collapse profile text | `lib/core/cors` (`LISTEN_HOST`/`isLocalHost`/`requireLocalHost`/`verifyUpgrade`/`setCors`/`PREVIEW_CSP`) / `lib/core/html` (`escapeHtml(s)` — five characters, no modes) / `lib/capture/profiles/util` | hardcode `127.0.0.1`, read `req.headers.host` by hand, copy the helper, or inline a `.replace` chain |
