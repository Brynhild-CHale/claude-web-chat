...
**Run `claude-web-chat open extensions`.** The daemon serves an install page that
prints the absolute path of this folder *on your machine* (with a copy button and
a zip download), the per-browser steps, and a live banner telling you whether the
capture hub is actually up. That page is the maintained instructions; the steps
below are the same thing without the machine-specific path.
...
The toolbar icon and the 16/32/48/128 px manifest icons are generated by
`node extensions/make-icons.js` — that script, not the PNGs, is the source.

## Use

- Click the toolbar icon → **Capture & send**, or right-click a page → **Send tab
  to web-chat**.
- The right-click path has no popup to report into, so it reports three other
  ways: a ✓/! badge on the toolbar icon (with the message as its tooltip), a
  toast injected into the page, and a record the popup shows next time you open
  it. A failed right-click capture is never silent.
- If nothing is listening on the hub port, the popup says so and prints the
  command to fix it (`claude-web-chat open`) instead of a bare "Failed to fetch".
- The capture lands as a card on the surface (owner `service:tab-stream`) and as
  a `tab_capture` signal in the right-edge queue rail. Hit **Push → Claude** (`P`)
  to wake Claude with it, or just mention it on your next prompt and Claude reads
  it with `get_captures`.
...
| Token | _(blank)_ | Required only if the server sets `WEB_CHAT_CAPTURE_TOKEN` or `.web-chat/capture-token`. Stored in `chrome.storage.local`, so it stays on this machine — the other two fields sync across your browsers, a shared secret must not. A token saved by an older build is moved out of sync the first time this one reads it. |
...
- Max body size is the server's JSON limit (`WEB_CHAT_BODY_LIMIT`, default
  `200mb`, applied by both the hub and the instance); very large pages may be
