...
- **First run waits on a TERMINAL approval — tell the user the command.** A service is host code running on the user's machine, so it does not start until they run `claude-web-chat trust <name>` in their terminal (bare `claude-web-chat trust` lists what is waiting; `--deny` refuses and stops the asking). The surface only shows a notice naming the command; it cannot grant the approval, because the component's own pane script runs in that page. Consent is recorded per **(project root, `service.js` contents, params)** in `~/.web-chat/services/trusted.json`, so editing the service, using it in another project, or spawning it with different params each ask again. If you mount a service-backed pane and don't say the command, the pane just sits there empty. Nothing runs headless (no viewer) or off the active node — navigating away stops the service, navigating back respawns it.
